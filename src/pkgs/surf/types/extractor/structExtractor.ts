...
> = {
  [TField in ExtractStructFieldsName<TABI, TStructName>]: ConvertStructFieldType<
    TABITable,
...
type ExtractStructFieldsName<T extends ABIRoot, TStructName extends ResourceStructName<T>> = ExtractStruct<
  T,
  TStructName
>["fields"][number]["name"];

type ExtractStruct<T extends ABIRoot, TResourceName extends ResourceStructName<T>> = Extract<
  Struct<T>,
  { name: TResourceName }
>;
...
> = Extract<ExtractStruct<T, TStructName>["fields"][number], { name: TFieldName }>["type"];

type ExtractStructMoveGenericArgsType<T extends ABIRoot, TStructName extends ResourceStructName<T>> = ExtractStruct<
  T,
  TStructName
>["generic_type_params"];
